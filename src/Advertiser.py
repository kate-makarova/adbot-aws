...
class Advertiser:
    def __init__(self, aws_id, aws_key, user_dir="/home/root/vipers/profile", log_mode='console', channel=None, session_id=None):
        options = Options()
      ...
      #  self.channel = channel
       # self.group_name = 'comm_' + session_id
        ...
        self.custom_l = False
        self.model = tf.keras.models.load_model('./src/model-2024-03-24.keras')
        self.templates = []
        self.forum_settings = {}
       ...
       # options.add_argument("user-data-dir=" + user_dir)
        self.driver1 = webdriver.Chrome(options=options, service=Service(executable_path='/usr/local/bin/chromedriver-linux64/chromedriver'))

        #options.add_argument("user-data-dir=/home/root/vipers/profile")
        self.driver2 = webdriver.Chrome(options=options, service=Service(executable_path='/usr/local/bin/chromedriver-linux64/chromedriver'))

    def load_forum_settings(self, home_forum_id):
        with open('./src/settings/home_forum.json') as f:
            d = json.load(f)
            self.forum_settings['ad_topic_template'] = d['ad_topic_template']
    ...
    def toInt(self, el):
        d = {}
        for n, val in el.items():
            if n in ['domain', 'custom_login']:
                d[n] = val
            else:
                d[n] = int(val)
        return d
    ...
    def load_forums(self, home_forum_id):
        ...
        response = table.query(
            IndexName='stop-activity-index',
            KeyConditionExpression=Key('stop').eq(0) & Key('activity').gte(0),
            FilterExpression=Attr('id').ne(home_forum_id),
    ...
    def work(self, url, id, home_forum_id, stop_list=False, templates=False, custom_login_code={}):
        ...
        self.load_forums(home_forum_id)
        self.log(total=str(0), success=str(0), skipped=str(0), visited=str(0),
                 message='Starting')
